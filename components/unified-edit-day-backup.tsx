...
export function UnifiedEditDay({ cycleId, dayId, onSave, onCancel }: UnifiedEditDayProps) {
  ...
  useEffect(() => {
    ...
    if (currentDay) {
      ...
      const daySpecific = unifiedMeds.daySpecific.map(med => {
        const isCustomMedication = !commonMedications.includes(med.name)
        return {
          id: med.id,
          name: isCustomMedication ? "custom" : med.name,
          customName: isCustomMedication ? med.name : "",
          dosage: med.dosage,
          hour: med.hour,
          minute: med.minute,
          ampm: med.ampm,
          refrigerated: med.refrigerated,
          taken: false, // Always start as not taken in edit form
          notes: med.notes
        }
      })
  ...
  const handleSubmit = () => {
    ...
    const updatedDaySpecificMeds = daySpecificMedications.map(med => ({
      ...
      ampm: med.ampm,
      refrigerated: med.refrigerated,
      taken: false, // Day-specific medications start as not taken
      skipped: false,
      takenAt: undefined,
      notes: med.notes
    }))
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1 space-y-3">
                      <div>
                        ...
                        <Select
                          value={med.name}
                          onValueChange={(value) => updateDaySpecificMedication(index, "name", value)}
                        >
                          <SelectTrigger>
                            <SelectValue placeholder="Select medication" />
                          </SelectTrigger>
                          <SelectContent>
                            {commonMedications.map((medName) => (
                              <SelectItem key={medName} value={medName}>
                                {medName}
                              </SelectItem>
                            ))}
                      ...
                      {med.name === "custom" && (
                        <div>
                          <Label htmlFor={`day-med-custom-name-${index}`}>Custom Medication Name</Label>
                          <Input
                            id={`day-med-custom-name-${index}`}
                            value={med.customName || ""}
                            onChange={(e) => updateDaySpecificMedication(index, "customName", e.target.value)}
